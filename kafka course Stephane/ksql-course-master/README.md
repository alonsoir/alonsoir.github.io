...
select countrycode, countryname from countrytable where countrycode='UK' emit changes limit 1;
...
CREATE TABLE VENDOR_INFORMATION (VENDOR_ID INT PRIMARY KEY,NAME VARCHAR) WITH (KAFKA_TOPIC='VENDOR_INFO', REPLICAS=1,PARTITIONS=1,VALUE_FORMAT='AVRO');
INSERT INTO PRODUCT_SUPPLY VALUES(1,'NAME',11,141,ARRAY[1,2,3]);
INSERT INTO PRODUCT_INFORMATION values (1,'151','abc',5);
INSERT INTO VENDOR_INFORMATION values (5,'VENDOR5');

CREATE STREAM PRODUCT_SUPPLY_INFORMATION AS
...
CREATE STREAM PRODUCT_SUPPLY_INFO_VENDOR_NAME AS
SELECT PSI.PRODUCT_ID AS PRODUCT_ID,
      STRUCT(NAME        := PSI.NAME,
              STOCK       := PSI.STOCK,
              PRICE       := PSI.PRICE,
              STORAGE_IDS := PSI.STORAGE_IDS) AS PRODUCT_SUPPLY,
      STRUCT(DESCRIPTION  := PSI.DESCRIPTION,
              MANUFACTURER := PSI.MANUFACTURER,
              VENDOR_ID    := PSI.VENDOR_ID) AS PRODUCT_INFORMATION,
      STRUCT(VENDOR_ID     := PSI.VENDOR_ID,
             NAME          := VI.NAME) AS VENDOR_INFORMATION
FROM PRODUCT_SUPPLY_INFORMATION PSI,
INNER JOIN VENDOR_INFORMATION VI
ON PSI.VENDOR_ID=VI.VENDOR_ID
GROUP BY PRODUCT_ID
EMIT CHANGES LIMIT 1;
